  ...
  putOrMkdirMultiple(inputs) {
    return new Promise((resolve, reject) => {
      ...
      inputs.forEach(input => {
        ...
        uploadPromises.push(this.putAwait(data, destPath));
      });

      Promise.all(checkingPaths.map(p => this.listAwait(p)))
        .then(
          async lists => {
            lists.forEach((list, index) => {
              if (list.length === 0) {
                creatingPromises.push(this.mkdirAwait(checkingPaths[index], true));
              }
            });
