...
class Qiniu {
    ...
    static haveUploadParamsReady(bucket, path = '', params = {}) {
        const uploadingFileName = params.fileName && params.fileName !== '' ? params.fileName : Qiniu.getRandomKey(path);
        let uploadParams = {
        ...
        if (params.returnUrl && params.returnBody) {
            ...
            uploadParams.returnBody = typeof params.returnBody === 'string' ? params.returnBody : JSON.stringify(params[returnBody]);
        }
        ...
        if (params.callbackUrl && params.callbackBody) {
            ...
            switch (callbackBodyType) {
                ...
                case 'APPLICATION/X-WWW-FORM-URLENCODED':
                    ...
                    uploadParams.callbackBody = typeof callbackBody === 'string' ? params.callbackBody : Qiniu.buildQueryString(params.callbackBody);
                    break;
                case 'JSON':
                case 'APPLICATION/JSON':
                default:
                    uploadParams.callbackBodyType = 'application/json';
                    uploadParams.callbackBody = typeof callbackBody === 'string' ? params.callbackBody : JSON.stringify(params.callbackBody)
            }
    ...
    static getRandomKey(orgUrl = '', prefix = '') {
        ...
        const randomedText = orgUrl === '' ? randomText + (Math.random() * 200) : randomText + (Math.random() * 99) + orgUrl;
        const randomKey = (crypto.createHash('md5').update(randomedText).digest('hex').substring(0, 17) + randomText).replace(/\-/, '_');
